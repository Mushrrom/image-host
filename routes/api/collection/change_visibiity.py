...
app = Blueprint('api/collection/change_visibility', __name__)



@app.route("/api/collection/<collection_id>/change_visibility", methods=["POST"])
def view_images(collection_id):
    if not "new_visibility" in request.form:
    ...
    # convert submitted new visibility to int
    try:
        new_visibility = int(request.form["new_visibility"])
    except:
        return {"success": 0, "error": "invalid request"}

    if not new_visibility in [0, 1]:
        return {"success": 0, "error": "invalid request"}
    ...
    # collection
    if not record:
        return {"success": 0, "error": "You dont have access to change the visibility of collection"}

    # If nothing else has had an error we can add the user
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()

    cursor.execute("""UPDATE imageCollection SET public = ? WHERE collectionID = ?""",
                    (new_visibility, collectionID))
    conn.commit()
    conn.close
